...
#import "MapErrorVisualizer.h"

@interface MapErrorVisualizer ()
...
@implementation MapErrorVisualizer
...
- (void)buildPipelines
{
    ...
    id<MTLFunction> kernelFunction = [library newFunctionWithName:@"mapErrorVisualizer"];
...
{
    if(!firstMapBuffer||!secondMapBuffer||!outErrorTexture){
        return;
    }
    ...
    commandBuffer.label = @"MapErrorVisualizerCommand";
    
    id<MTLComputeCommandEncoder> computeEncoder = [commandBuffer computeCommandEncoder];
    
    [computeEncoder setComputePipelineState:_computePipeline];
    
    [computeEncoder setBuffer:firstMapBuffer offset:0 atIndex:0];
    
    [computeEncoder setBuffer:secondMapBuffer offset:0 atIndex:1];
    
    [computeEncoder setTexture:outErrorTexture atIndex:0];
