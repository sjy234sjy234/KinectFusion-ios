...
#import "DepthMapGenerator.h"

@interface DepthMapGenerator ()
...
@property (nonatomic, strong) id<MTLBuffer> cameraNDC2DepthBuffer;

@end

@implementation DepthMapGenerator
...
- (void)buildPipelines
{
    ...
    id<MTLFunction> kernelFunction = [library newFunctionWithName:@"depthMapGenerator"];
    ...
    _cameraNDC2DepthBuffer = [_metalContext.device newBufferWithLength:sizeof(CameraNDC2Depth)
...
{
    if(!currentDisparityPixelBuffer||!currentDepthMapBufferPyramid||!preDepthTexture||!preDepthMapBufferPyramid){
        return;
    }
    
    id<MTLBuffer> inDisparityMapBuffer = [self bufferWithF16PixelBuffer:currentDisparityPixelBuffer];
    
    memcpy([_cameraNDC2DepthBuffer contents], &cameraNDC2Depth, sizeof(CameraNDC2Depth));
    
    id<MTLCommandBuffer> commandBuffer = [_metalContext.commandQueue commandBuffer];
    commandBuffer.label = @"DepthMapGeneratorCommand";
    ...
    [computeEncoder setBuffer:currentDepthMapBufferPyramid[0] offset:0 atIndex:1];
    
    [computeEncoder setBuffer:currentDepthMapBufferPyramid[1] offset:0 atIndex:2];
    
    [computeEncoder setBuffer:currentDepthMapBufferPyramid[2] offset:0 atIndex:3];
    
    [computeEncoder setTexture:preDepthTexture atIndex:0];
    
    [computeEncoder setBuffer:preDepthMapBufferPyramid[0] offset:0 atIndex:4];
    
    [computeEncoder setBuffer:preDepthMapBufferPyramid[1] offset:0 atIndex:5];
    
    [computeEncoder setBuffer:preDepthMapBufferPyramid[2] offset:0 atIndex:6];
    
    [computeEncoder setBuffer:_cameraNDC2DepthBuffer offset:0 atIndex:7];
